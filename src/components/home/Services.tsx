  ...
  return <div className="animate-fade-up" style={{
    animationDelay: `${delay}ms`
      ...
      <GlassMorphismCard variant="interactive" padding="sm" className="h-full flex flex-col relative overflow-hidden">
        ...
        <Link to={link}>
          <AnimatedButton variant="primary" size="md" fullWidth iconRight={<ArrowRight size={16} />} className="bg-teal-600 hover:bg-teal-700 text-white border-0">
            {cta}
          </AnimatedButton>
        </Link>
      </GlassMorphismCard>
...
const getServiceIcon = (category: string) => {
  switch (category.toLowerCase()) {
    case 'deck':
      return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 text-teal-600">
  ...
  return <section id="services" ref={servicesRef} className="bg-zinc-50 py-[42px]">
      <div className="container px-4 mx-auto">
        ...
        {isLoading ? <div className="flex justify-center py-8">
          ...
          </div> : services.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => <ServiceCard key={index} {...service} />)}
          </div> : <div className="text-center py-8">
            <p className="text-muted-foreground">No services available at the moment.</p>
          </div>}

        <div className="mt-16 text-center animate-fade-up" style={{
